...
#This file will is the frontend(GUI of the app)
import PMC_app4_bk
from tkinter import *
...
def view_command():
    lstb1.delete(0,END)
    for row in PMC_app4_bk.view_db():
...
def search_command():
    ...
    for row in PMC_app4_bk.search_db(title_value.get(), author_value.get(), year_value.get(), isbn_value.get()):
        lstb1.insert(END, row)

#Create an add function for "insert_db"
def add_command():
    PMC_app4_bk.insert_db(title_value.get(), author_value.get(), year_value.get(), isbn_value.get())
    lstb1.delete(0,END)
...
def delete_command():
    PMC_app4_bk.delete_db(selected_tuple[0])
...
def update_command():
    PMC_app4_bk.update_db(selected_tuple[0], title_value.get(), author_value.get(), year_value.get(), isbn_value.get())
